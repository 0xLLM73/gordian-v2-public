...
function helperPath(command: string, args: string[]): string {
	return command === 'swift' ? (args[2] ?? '') : command;
}
...
function keychainGetCalls() {
	return spawnMock.mock.calls.filter(
		([command, args]) =>
			...
			Array.isArray(args) &&
			helperPath(command, args).includes('keychain-get-secret'),
	);
}
...
beforeEach(() => {
	...
	Reflect.deleteProperty(process.env, 'TELEGRAM_KEYCHAIN_USER_PRESENCE_MODE');
	Reflect.deleteProperty(process.env, 'GORDIAN_KEYCHAIN_HELPER_PATH');
	...
	spawnMock.mockImplementation((command: string, args: string[]) => {
		const isSwiftRunner = command === 'swift';
		const isCompiledHelper = command.includes('keychain-');
		const isUnifiedHelper = command.endsWith('/gordian-keychain-helper');
		if (!isSwiftRunner && !isCompiledHelper) throw new Error(`Unexpected command ${command}`);
		const service = isSwiftRunner ? args[3] : isUnifiedHelper ? args[1] : args[0];
		const account = isSwiftRunner ? args[4] : isUnifiedHelper ? args[2] : args[1];
		const mode = isSwiftRunner ? args[5] : isUnifiedHelper ? args[3] : args[2];
		if (!service || !account || !mode) throw new Error('Missing keychain helper arguments');
		const isGetHelper =
			helperPath(command, args).includes('keychain-get-secret') ||
			(isUnifiedHelper && args[0] === 'get');
...
describe('OS keychain Telegram session KEK provider', () => {
	...
	it('uses separate default Keychain services for workspace and Telegram secrets', async () => {
		Reflect.deleteProperty(process.env, 'TELEGRAM_KEYCHAIN_SERVICE');
		Reflect.deleteProperty(process.env, 'WORKSPACE_KEYCHAIN_SERVICE');
		const { generateSessionKek, generateWorkspaceWrk } = await import('../kms');

		const telegram = await generateSessionKek('user-default-services');
		const workspace = await generateWorkspaceWrk('workspace-default-services');
		const telegramMarker = JSON.parse(
			telegram.ciphertextBlob
				.toString('utf8')
				.slice('gordian:keychain:telegram-session-kek:v1:'.length),
		) as { service: string };
		const workspaceMarker = JSON.parse(
			workspace.encryptedWrk.toString('utf8').slice('gordian:keychain:workspace-wrk:v1:'.length),
		) as { service: string };

		expect(telegramMarker.service).toBe('gordian-v2-telegram');
		expect(workspaceMarker.service).toBe('gordian-v2-workspace');
	});
	...
	it('passes user-presence mode to Telegram session keychain helpers', async () => {
		...
		expect(spawnMock).toHaveBeenCalledWith(
			expect.stringContaining('keychain-set-secret'),
	...
	it('uses a temporary helper when strict user-presence lacks a configured broker', async () => {
		process.env.TELEGRAM_KEYCHAIN_REQUIRE_USER_PRESENCE = 'true';
		process.env.TELEGRAM_KEYCHAIN_USER_PRESENCE_MODE = 'strict';
		const { decryptSessionKek, generateSessionKek } = await import('../kms');

		const { ciphertextBlob } = await generateSessionKek('user-presence-strict');
		expect(spawnMock).toHaveBeenCalledWith(
			expect.stringContaining('keychain-set-secret'),
			expect.arrayContaining(['strict-user-presence']),
			expect.any(Object),
		);

		await decryptSessionKek(ciphertextBlob, 'user-presence-strict');
		expect(keychainGetCalls()[0]?.[1]).toEqual(expect.arrayContaining(['strict-user-presence']));
	});

	it('uses a configured packaged helper for nonstandard keychain modes', async () => {
		process.env.TELEGRAM_KEYCHAIN_REQUIRE_USER_PRESENCE = 'true';
		process.env.TELEGRAM_KEYCHAIN_USER_PRESENCE_MODE = 'strict';
		process.env.GORDIAN_KEYCHAIN_HELPER_PATH = '/tmp/gordian-keychain-helper';
		const { decryptSessionKek, generateSessionKek } = await import('../kms');

		const { ciphertextBlob } = await generateSessionKek('user-packaged-helper');
		expect(spawnMock).toHaveBeenCalledWith(
			'/tmp/gordian-keychain-helper',
			expect.arrayContaining(['set', 'gordian-test', expect.any(String), 'strict-user-presence']),
			expect.any(Object),
		);

		await decryptSessionKek(ciphertextBlob, 'user-packaged-helper');
		expect(spawnMock).toHaveBeenCalledWith(
			'/tmp/gordian-keychain-helper',
			expect.arrayContaining(['get', 'gordian-test', expect.any(String), 'strict-user-presence']),
			expect.any(Object),
		);
	});

	it('returns a new Telegram marker when rehardening moves to a new Keychain service', async () => {
		process.env.TELEGRAM_KEYCHAIN_SERVICE = 'gordian-old-telegram';
		const { generateSessionKek, rehardenSessionKek } = await import('../kms');

		const { ciphertextBlob } = await generateSessionKek('user-rehome');
		process.env.TELEGRAM_KEYCHAIN_SERVICE = 'gordian-new-telegram';
		const updatedBlob = await rehardenSessionKek('user-rehome', ciphertextBlob);

		expect(updatedBlob).toBeDefined();
		const updatedMarker = JSON.parse(
			updatedBlob?.toString('utf8').slice('gordian:keychain:telegram-session-kek:v1:'.length) ??
				'{}',
		) as { account: string; service: string };
		expect(updatedMarker.service).toBe('gordian-new-telegram');
		expect(keychain.has(`${updatedMarker.service}:${updatedMarker.account}`)).toBe(true);
	});
});
...
describe('OS keychain workspace WRK provider', () => {
	...
	it('reads workspace WRKs without an operation prompt in standard mode', async () => {
		const { generateWorkspaceWrk, unwrapWrk } = await import('../kms');

		const envelope = {
			...(await generateWorkspaceWrk('workspace-silent-read')),
			wrkVersion: 1,
		};

		await unwrapWrk(envelope);

		const getArgs = keychainGetCalls()[0]?.[1] as string[] | undefined;
		expect(getArgs?.[getArgs.length - 2]).toBe('standard');
		expect(getArgs?.[getArgs.length - 1]).toBe('');
	});
