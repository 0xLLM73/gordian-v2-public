-- Migration: 0041_iterative_scan.sql
-- KG-1: Enable iterative HNSW scanning in hybrid_search() to prevent
-- overfiltering in multi-tenant queries. Requires pgvector 0.8.0+.
--
-- Without this, the HNSW index finds N globally nearest vectors first,
-- THEN applies WHERE workspace_id = ?, discarding other workspaces'
-- results and degrading recall as the table grows.
...
BEGIN
  -- KG-1: Enable iterative scan to prevent HNSW overfiltering in multi-tenant queries
