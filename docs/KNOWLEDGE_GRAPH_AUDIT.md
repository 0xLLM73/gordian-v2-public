...
### Historical Validation Commands Run

This table records the local state from the original knowledge-graph audit. It
is not current public-release evidence. For the current release gate, rerun the
commands in [PUBLISHING.md](PUBLISHING.md) from the selected publication
checkout.
...
| `pnpm audit:open-source` | Superseded by current release gate | This historical audit was written before the root script existed. The current public-readiness gate is defined in `package.json`, `README.md`, and [PUBLISHING.md](PUBLISHING.md). |
