...
| Validated source baseline | App-code baseline remains `c110d801ad769040f788820c4c6bfd5dbf56bae0`; source `main` is current through evidence-only PRs #117, #118, #119, and #120 at `fa6578c30e53d5c52d7479f24d6e1d7297bfc2a4` |
...
| Publication target | `0xLLM73/gordian-v2-public`; private mirror PR #38 synced source commit `c110d801ad769040f788820c4c6bfd5dbf56bae0`, and mirror PRs #39, #40, and #41 synced later evidence-only release docs through mirror `main` `26c856c9ae9641e4464b26294522f6e75bc1df02` after green CI and required `@thegrovest` review |
...
through #120 are evidence-only documentation updates, and mirror PRs #38 through
#41 reran the required gates from the mirror checkout before merge, but this is
...
| --- | --- | --- | --- |
| Clean checkout | `git status --short --branch` shows a clean release branch | PASS | 2026-06-10 PT: source `main` was clean and synced to `origin/main` at `fa6578c30e53d5c52d7479f24d6e1d7297bfc2a4`; mirror `main` was clean and synced to `origin/main` at `26c856c9ae9641e4464b26294522f6e75bc1df02`. |
...
| Telegram local security smoke | `pnpm telegram:security-smoke --allow-missing-credentials` passes against local DB/Redis | PARTIAL | 2026-06-10 PT: skipped-credential local smoke completed with 0 failures and 2 warnings after moving 3 local workspace WRKs to macOS Keychain markers. After browser QA reseeded demo data, `pnpm workspace-key:migrate-local-keychain -- --apply` migrated the 3 seeded WRKs again, and the follow-up smoke still reported 0 failures and 2 warnings. DB and Redis residue scans passed; remaining warnings were skipped Telegram API Keychain credential presence and worker route not reachable. Credentialed Telegram API presence and worker send-disabled route sign-off remain pending. Mirror PR #38 used temporary nontracked env `/private/tmp/gordian-mirror-telegram-smoke.env`. |
...
| Publication check | `pnpm check:publication --repo 0xLLM73/gordian-v2-public` passes after the repo is public | BLOCKED | 2026-06-10 PT: post-PR #41 mirror check failed as expected because the selected mirror is private; secret scanning and push protection are unavailable; private vulnerability reporting returns 404. Rerun after the sanitized mirror is synced, intentionally made public, and GitHub-side settings are enabled. |
...
| Branch checks | `main` requires strict `validate` and `demo-smoke` checks | PASS | Source and mirror `main` require strict `validate` and `demo-smoke` checks. Mirror PRs #38, #39, #40, and #41 passed GitHub `validate` and `demo-smoke`; PR #38 also passed `postgres-smoke`. |
| Review policy | `main` requires owner approval before merge | PASS | 2026-06-10 PT: source and mirror branch protection require CODEOWNER review with one approval, dismiss stale reviews, enforce admins, require linear history, block force pushes, and block deletions. Mirror PRs #38 through #41 stayed blocked until required `@thegrovest` review landed, then merged cleanly. |
...
| Gate | Required result | Status | Evidence |
| --- | --- | --- | --- |
| Every PR gate | CI runs open-source audit, package audits, lint, typecheck, tests, and demo smoke before merge | PASS | PRs #108 through #120 merged only after required review and green CI; mirror PRs #38 through #41 merged after required review and green CI. |
...
| Gate | Required result | Status | Evidence |
| --- | --- | --- | --- |
| Runtime purge | `pnpm purge:secrets` run where old real runtime state existed | PARTIAL | 2026-06-10 PT: `pnpm purge:secrets -- --dry-run` printed counts only. The latest post-browser-QA dry-run found 0 Telegram MTProto account sessions/tokens, 0 account OAuth tokens, 3 Better Auth demo sessions, 0 verification values, 0 calendar OAuth tokens, and 0 Telegram/phone/session Redis keys. It found 1911 `{ai-flow}:*` BullMQ runtime keys covered by purge tooling; destructive `--confirm` purge still requires release-owner approval or explicit owner acceptance of this local non-release residue. |
...
| Telegram smoke | `pnpm telegram:security-smoke` passes against the intended local environment | PARTIAL | 2026-06-10 PT: `pnpm telegram:security-smoke --allow-missing-credentials` passed with 0 failures and 2 warnings after local workspace WRK migration, including a rerun after browser QA reseeded demo data. Credentialed Telegram API presence and worker send-disabled route verification remain pending before any real-account release test. |
