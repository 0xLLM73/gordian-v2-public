...
MTProto session unwrap keys must use `TELEGRAM_SESSION_KEY_PROVIDER=os-keychain` or `TELEGRAM_SESSION_KEY_PROVIDER=aws-kms` before `TELEGRAM_MTPROTO_ENABLED=true`. The `os-keychain` provider stores the unwrap key in macOS Keychain and stores only a marker in Postgres. Run `pnpm telegram:keychain:harden` for already linked accounts so existing Keychain items are re-stored as `WhenUnlockedThisDeviceOnly`. `TELEGRAM_KEYCHAIN_REQUIRE_USER_PRESENCE=true` enables the additional user-presence path. Set `TELEGRAM_KEYCHAIN_USER_PRESENCE_MODE=strict` to require macOS `SecAccessControl.userPresence` after `pnpm telegram:touchid:probe` passes, or `compat` as a fallback when strict prompts are unavailable. `GORDIAN_KEYCHAIN_HELPER_PATH` is optional; it routes strict reads through a branded helper so prompts identify Gordian. The `aws-kms` provider stores an AWS KMS data-key ciphertext blob in Postgres. The `dev-insecure` provider is blocked for MTProto because it stores the unwrap key beside the encrypted session.

Use separate local Keychain services for workspace keys and Telegram session keys, for example `WORKSPACE_KEYCHAIN_SERVICE=gordian-v2-workspace` and `TELEGRAM_KEYCHAIN_SERVICE=gordian-v2-telegram`. Dashboard/local-data unlocks are workspace-key reads; explicit Telegram history imports are session-key reads.

The recommended usable local mode is `TELEGRAM_MTPROTO_PER_INTERACTION_UNLOCK=false`: one user-presence unlock starts or resumes an import, and completed, paused, cancelled, and finally failed imports disconnect the local Telegram client. The helper thread may stay alive until `TELEGRAM_MTPROTO_SESSION_IDLE_MINUTES` so repeat imports avoid extra Telegram API credential Keychain prompts. If the local Telegram client closes unexpectedly during an active run, the run fails with a resume-required message instead of silently reading the Keychain unwrap key again. Set it to `true` only for maximum per-read prompts. Optional helper packaging can improve prompt identity and distribution, but strict local user-presence gating is verified by `pnpm telegram:touchid:probe`, not by committing signing material. See [macOS Touch ID for Telegram MTProto](./MACOS_TOUCH_ID_MTPROTO.md).

`TELEGRAM_MTPROTO_SESSION_IDLE_MINUTES` controls how long a local GramJS helper thread may remain alive while idle. Use `5` minutes for personal-account testing. The history import worker disconnects the Telegram client when an import run completes, is cancelled, or finally fails, and idle eviction later terminates the helper thread. Eviction drops remaining helper-thread memory, but it does not revoke the linked Telegram session or delete the encrypted session row.
