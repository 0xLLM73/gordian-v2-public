...
Before publishing, sync the selected `0xLLM73/gordian-v2-public` mirror as a
sanitized release tree from the private source repository, verify absolute
GitHub links point at the mirror, and rerun the full release gate from that
mirror checkout. Do not publish private source history unless it has been
separately approved for public release.
...
10. Use generated local secrets.
...
Use `TELEGRAM_SESSION_KEY_PROVIDER=os-keychain` for local macOS personal-account testing. The Telegram session unwrap key stays in macOS Keychain and only a marker is stored in Postgres. Run `pnpm telegram:keychain:harden` for existing local links so their Keychain items use `WhenUnlockedThisDeviceOnly`. `TELEGRAM_KEYCHAIN_REQUIRE_USER_PRESENCE=true` with `TELEGRAM_KEYCHAIN_USER_PRESENCE_MODE=strict` protects the unwrap key with macOS `SecAccessControl.userPresence`; run `pnpm telegram:touchid:probe` before relying on it. Keep `TELEGRAM_MTPROTO_PER_INTERACTION_UNLOCK=false` for one user-presence unlock per import run; the worker disconnects the local Telegram client when the import completes, pauses, cancels, or finally fails, while the helper thread can remain alive until the idle timeout to avoid repeated Telegram API credential prompts. If the client closes unexpectedly during a run, resume the import to perform a new explicit unlock instead of silently re-reading the Keychain item. Set it to `true` only for stricter per-read prompts. Use `aws-kms` with `KMS_CMK_ARN` and AWS credentials for a KMS-backed setup. The worker refuses to enable MTProto with `dev-insecure`. See [macOS Touch ID for Telegram MTProto](./MACOS_TOUCH_ID_MTPROTO.md) for optional helper packaging and public-release signing guidance.

Set `TELEGRAM_MTPROTO_SESSION_IDLE_MINUTES=5` for personal-account testing. This terminates idle GramJS helper threads and drops any remaining helper-thread memory after 5 idle minutes, without revoking or deleting the linked Telegram session. Terminal import cleanup disconnects the Telegram client before the idle timeout. An active import does not silently unlock again after eviction; resume it to approve another explicit unlock.
...
Use `WORKSPACE_KEY_PROVIDER=os-keychain` for real local macOS workspaces. This keeps the workspace root key for encrypted messages, commitments, memories, and related local data in macOS Keychain while `workspaces.encrypted_wrk` stores only a marker. Keep `WORKSPACE_KEYCHAIN_SERVICE` separate from `TELEGRAM_KEYCHAIN_SERVICE` so dashboard/local-data unlocks are distinct from Telegram import-session unlocks. Run `pnpm workspace-key:migrate-local-keychain -- --apply` to move existing local dev workspace keys out of Postgres. Set `WORKSPACE_KEY_CACHE_TTL_MINUTES=60` for smoother local use, or a lower value for stricter memory exposure.
