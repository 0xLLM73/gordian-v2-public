...
`TELEGRAM_MTPROTO_SESSION_IDLE_MINUTES` controls how long the local GramJS worker may keep a decrypted Telegram client/session in process memory after the last action. Use `5` for personal-account testing. When the worker is evicted, the encrypted session remains linked in Postgres and the next import can reconnect by reading the session unwrap key from Keychain again. This is not a Telegram-side revocation.
...
`TELEGRAM_KEYCHAIN_REQUIRE_USER_PRESENCE="true"` attempts to add per-access Touch ID/password prompts for Telegram session unwrap. Some unsigned local helper processes fail with macOS `errSecMissingEntitlement`; keep it `false` unless you have verified the local packaged helper supports user-presence access control.
