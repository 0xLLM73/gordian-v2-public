...
| `TELEGRAM_KEYCHAIN_SERVICE` | CONFIG | Only if `TELEGRAM_SESSION_KEY_PROVIDER=os-keychain` | `gordian-v2` | macOS Keychain service name used for Telegram session unwrap keys. The database stores only a marker for these keys. | personal-tg |
| `TELEGRAM_KEYCHAIN_REQUIRE_USER_PRESENCE` | CONFIG | No | `false` | Attempts per-access Touch ID/password prompts for Telegram session Keychain reads. Keep false unless the packaged local helper has been verified; unsigned helpers can fail with macOS entitlement errors. | personal-tg |
...
| `WORKSPACE_KEYCHAIN_SERVICE` | CONFIG | Only if `WORKSPACE_KEY_PROVIDER=os-keychain` | Falls back to `GORDIAN_KEYCHAIN_SERVICE`, `TELEGRAM_KEYCHAIN_SERVICE`, then `gordian-v2` | macOS Keychain service name used for workspace root keys. | local-security |
...
| `WORKSPACE_KEYCHAIN_SERVICE` | CONFIG | Only if `WORKSPACE_KEY_PROVIDER=os-keychain` | `gordian-v2` via fallback chain | macOS Keychain service for workspace root-key items. | local-security |
...
WORKSPACE_KEYCHAIN_SERVICE="gordian-v2"
...
TELEGRAM_KEYCHAIN_SERVICE="gordian-v2"
