	...
	useEffect(() => {
		...
		async function recoverOnboardingState() {
			try {
				...
				setTelegramImportOnlyMode(data.telegramImportOnlyMode === true);
		...
		<OnboardingCard className="max-w-xl">
			...
			{syncStarted ? (
				...
				<div className="space-y-4">
					{telegramImportOnlyMode ? (
						<div className="space-y-4">
							<div className="rounded-lg border border-border bg-muted/40 px-4 py-3 text-sm text-muted-foreground">
								This local build requires the Telegram session to open only inside the history
								import flow. Continue to the dashboard to start an import with macOS approval.
							</div>
