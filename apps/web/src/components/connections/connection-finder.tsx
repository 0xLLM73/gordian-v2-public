...
export function ConnectionFinder() {
	...
	const { executeAsync, isExecuting } = useAction(findConnectionsForPeriodAction);
	...
	async function estimateRun() {
		...
		toast.error(result?.serverError ?? 'Failed to estimate connection search');
	}
	...
	async function queueRun() {
		...
		toast.error(result?.serverError ?? 'Failed to queue connection search');
	}
	...
	return (
		<section className="mb-4 rounded-lg border border-border bg-background p-4">
			<div className="flex flex-col gap-4">
				<div>
					<h3 className="text-sm font-semibold text-foreground">Find new connections</h3>
			...
			{lastQueued ? (
				<div className="mt-4 rounded-md border border-emerald-200 bg-emerald-50 px-3 py-2 text-sm text-emerald-900">
					...
					{lastQueued.contactsProcessed.toLocaleString()} contacts. Detected connections will appear
					below after the worker finishes.
				</div>
			) : null}
		</section>
	);
