...
import {
	telegramHistoryImportQueue,
	telegramHistoryImportWorker,
...
import { runtimeCleanup } from './routes/runtime-cleanup';
...
if (isTelegramMtProtoEnabled()) {
	assertSafeTelegramSessionKeyProviderForMtProto();
	console.log(`Telegram MTProto session key provider: ${getTelegramSessionKeyProvider()}`);
}
...
app.route('/runtime', runtimeCleanup);
...
async function main() {
	...
	void telegramHistoryImportWorker; // dashboard Telegram history import
	...
	const workerHost =
		...
		(process.env.NODE_ENV === 'production' || process.env.FLY_APP_NAME ? '0.0.0.0' : '127.0.0.1');
	const workerPort = Number(process.env.WORKER_PORT || 3001);
	serve({ fetch: app.fetch, hostname: workerHost, port: workerPort }, () => {
		console.log(`Gordian Worker running on ${workerHost}:${workerPort}`);
	});
