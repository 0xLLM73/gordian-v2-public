...
export interface CommitmentReprocessInput {
	workspaceId: string;
	userId: string;
	batchSize?: number;
	contactLimit?: number;
...
export interface CommitmentReprocessEstimate {
	workspaceId: string;
	contactLimit: number;
	batchSize: number;
...
export interface CommitmentReprocessQueueResult {
...
export function normalizeCommitmentReprocessContactIds(value: unknown): string[] | undefined {
...
export function normalizeCommitmentReprocessMaxAgeDays(value: unknown): number | undefined {
...
export function normalizeCommitmentReprocessBatchSize(value: unknown): number {
	return Math.min(Math.max(Number(value ?? 200), 1), 200);
}

export function normalizeCommitmentReprocessContactLimit(value: unknown): number {
...
async function getCommitmentReprocessContactCandidates(input: {
	workspaceId: string;
	contactLimit: number;
	sourceAccountId?: string;
	contactIds?: string[];
...
async function getCommitmentReprocessMessages(input: {
	workspaceId: string;
	contactId: string;
...
function normalizeCommitmentReprocessInput(input: CommitmentReprocessInput) {
	return {
		...input,
		batchSize: normalizeCommitmentReprocessBatchSize(input.batchSize),
		contactLimit: normalizeCommitmentReprocessContactLimit(input.contactLimit),
		contactIds: normalizeCommitmentReprocessContactIds(input.contactIds),
		maxAgeDays: normalizeCommitmentReprocessMaxAgeDays(input.maxAgeDays),
		sourceAccountId:
...
export async function estimateCommitmentReprocess(
	input: CommitmentReprocessInput,
): Promise<CommitmentReprocessEstimate> {
	const normalized = normalizeCommitmentReprocessInput(input);
	const workspaceContacts = await getCommitmentReprocessContactCandidates({
...
export async function queueCommitmentReprocess(
	input: CommitmentReprocessInput,
): Promise<CommitmentReprocessQueueResult> {
	const normalized = normalizeCommitmentReprocessInput(input);
	const workspaceContacts = await getCommitmentReprocessContactCandidates({
		workspaceId: normalized.workspaceId,
		contactLimit: normalized.contactLimit,
	...
	for (const contact of workspaceContacts) {
		const msgs = await getCommitmentReprocessMessages({
			workspaceId: normalized.workspaceId,
		...
		await scheduleAIPipeline(
			...
			{ skipWorkspaceRelationshipDerivation: normalized.skipWorkspaceRelationshipDerivation },
		);
