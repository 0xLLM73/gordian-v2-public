...
import {
	...
	getTelegramKeychainUserPresenceMode,
	parseArgs,
	parseEnvText,
	probeMacOsKeychain,
...
async function main() {
	...
	if (!args['skip-keychain']) {
		...
		const userPresenceMode = getTelegramKeychainUserPresenceMode(env);
		const helperPath =
			envValue(env, 'GORDIAN_KEYCHAIN_HELPER_PATH') ||
			process.env.GORDIAN_KEYCHAIN_HELPER_PATH ||
			'';
		try {
			if (requireUserPresence && userPresenceMode === 'strict') {
				await probeMacOsStrictTouchIdKeychain(service, { helperPath });
				checks.push({
					detail: `strict SecAccessControl userPresence probe passed for service ${service}`,
					level: 'pass',
					name: 'macOS Touch ID Keychain',
				});
			} else if (requireUserPresence) {
				checks.push({
					detail:
						'standard write/read/delete probe skipped because compat approval mode intentionally prompts on reads',
					level: 'warn',
					name: 'macOS Keychain',
				});
			} else {
